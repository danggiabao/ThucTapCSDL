...
MANV VARCHAR(5) REFERENCES NHANVIEN(MANV) ON DELETE CASCADE,
MAKH varchar(5) REFERENCES KHACHHANG(MAKH) ON DELETE CASCADE,
THANHTIEN INT,
)	
GO
...
TENMON NVARCHAR(50),
DONGIA INT,
MANM VARCHAR(5) REFERENCES NHOMMON(MANM) ON DELETE CASCADE
)
GO
...
THOIGIAN TIME,
MAKH VARCHAR(5) REFERENCES KHACHHANG(MAKH) ON DELETE CASCADE,
MABAN VARCHAR(5) REFERENCES BAN(MABAN) ON DELETE CASCADE
)
GO
CREATE TABLE CHITIETPHIEUYEUCAU(
MAPHIEU VARCHAR(5) REFERENCES PHIEUYEUCAU(MAPHIEU)ON DELETE CASCADE,
SOLUONG INT,
MAMON VARCHAR(5) REFERENCES MONAN(MAMON) ON DELETE CASCADE
PRIMARY KEY(MAPHIEU,MAMON)
)
Go
--INSERT
INSERT INTO TAIKHOAN VALUES ('Nam','admin','123456',1);
INSERT INTO TAIKHOAN VALUES ('a','a','a',1);
GO
...
GO
INSERT INTO KHACHHANG VALUES ('KH01',N'Lâm Xung');
INSERT INTO KHACHHANG VALUES ('KH02',N'Tống Giang');
INSERT INTO KHACHHANG VALUES ('KH03',N'Lư Tuấn nghĩa');
INSERT INTO KHACHHANG VALUES ('KH04',N'Võ Tòng');
INSERT INTO KHACHHANG VALUES ('KH05',N'Phan Kim Liên');
INSERT INTO KHACHHANG VALUES ('KH06',N'Lữ Bố');
INSERT INTO KHACHHANG VALUES ('KH07',N'Điêu Thuyền');
INSERT INTO KHACHHANG VALUES ('KH08',N'Tào Tháo');
GO

INSERT INTO NHANVIEN VALUES ('NV01',N'Sơn Tùng','Nam','1995-2-2',N'Thái Bình');
INSERT INTO NHANVIEN VALUES ('NV02',N'Chị Dậu',N'Nữ','1992-11-22',N'Hà Nội');
...
INSERT INTO NHANVIEN VALUES ('NV07',N'Thuỳ Linh',N'Nữ','12-22-1993',N'Sơn La');
INSERT INTO NHANVIEN VALUES ('NV08',N'Vàng Anh',N'Nữ','11-11-2002',N'Thái Bình');
GO
INSERT INTO HOADON VALUES ('HD01','3-02-2018','6:12','NV01','KH01',500000);
INSERT INTO HOADON VALUES ('HD02','11-02-2018','20:00','NV02','KH02',400000);
INSERT INTO HOADON VALUES ('HD03','01-01-2018','9:30','NV03','KH03',600000);
INSERT INTO HOADON VALUES ('HD04','07-09-2017','12:00','NV04','KH04',200000);
INSERT INTO HOADON VALUES ('HD05','07-09-2017','13:10','NV05','KH05',100000);
INSERT INTO HOADON VALUES ('HD06','07-09-2017','14:03','NV06','KH06',50000);
GO

INSERT INTO PHIEUYEUCAU VALUES ('PH01','11:11','KH01','001');
INSERT INTO PHIEUYEUCAU VALUES ('PH02','22:22','KH02','103');
INSERT INTO PHIEUYEUCAU VALUES ('PH03','12:21','KH03','102');
INSERT INTO PHIEUYEUCAU VALUES ('PH04','21:12','KH04','302');
GO
...
Go
--THÊM DỮ LIỆU
CREATE PROC ThemTK(@TENDANGNHAP NVARCHAR(50),@MATKHAU nvarchar(50),@PHANQUYEN INT) AS 
 BEGIN
...
CREATE PROC DoiMK(@matkhau varchar(50)) AS 
BEGIN
	UPDATE TAIKHOAN
	SET MATKHAU = @matkhau
END
...
-- bảng NHANVIEN 
	--Them
CREATE PROCEDURE ADD_NHANVIEN(@MANV VARCHAR(5),@HOTEN NVARCHAR(50),@GIOITINH NVARCHAR(5),@NGAYSINH DATE,@QUEQUAN NVARCHAR(50))
AS
BEGIN
...
GO
	--Sua
CREATE PROCEDURE ALTER_NHANVIEN(@MANV VARCHAR(5),@HOTEN NVARCHAR(50),@GIOITINH NVARCHAR(5),@NGAYSINH DATE,@QUEQUAN NVARCHAR(50))
AS
BEGIN
	UPDATE NHANVIEN
	SET HOTEN=@HOTEN,GIOITINH=@GIOITINH,NGAYSINH=@NGAYSINH,QUEQUAN=@QUEQUAN
	WHERE MANV=@MANV
			
END	
GO	
	--Xoa
CREATE PROCEDURE D_NHANVIEN(@MANV VARCHAR(5))
AS
BEGIN 
...
--bảng HOADON
	--Thêm
create PROCEDURE ADD_HOADON (@MAHD VARCHAR(5) ,@NGAYLAP DATE,@GIOTHANHTOAN DATETIME,@MANV VARCHAR(5),@MAKH VARCHAR(5),@THANHTIEN INT)
AS
BEGIN
	INSERT INTO HOADON
	VALUES(@MAHD,@NGAYLAP,@GIOTHANHTOAN,@MANV, @MAKH,@THANHTIEN)
END
GO
	--Sủa
create PROCEDURE ALTER_HOADON(@MAHD VARCHAR(5) ,@NGAYLAP DATE,@GIOTHANHTOAN DATETIME,@MANV VARCHAR(5),@MAKH varchar(5),@THANHTIEN INT )
AS
BEGIN
	UPDATE HOADON
	SET NGAYLAP=@NGAYLAP,GIOTHANHTOAN=@GIOTHANHTOAN,MANV=@MANV,MAKH=@MAKH,THANHTIEN=@THANHTIEN
	WHERE MAHD=@MAHD
END
GO
	--Xóa
CREATE PROCEDURE D_HOADON(@MAHD VARCHAR(5))
AS
BEGIN 
				DELETE FROM HOADON
				WHERE MAHD=@MAHD
...
as
Select NGAYLAP,SUM(THANHTIEN)as THANHTIEN from HOADON group by NGAYLAP
GO
--Thêm dữ liệu bảng KHACHHANG
CREATE PROCEDURE ADD_KHACHHANG (@MAKH VARCHAR(5),@HOTEN NVARCHAR(50))
AS
BEGIN
...
GO
	--Sua
CREATE PROCEDURE ALTER_KHACHHANG (@MAKH VARCHAR(5),@HOTEN NVARCHAR(50))
AS
BEGIN
IF EXISTS(SELECT @MAKH,@HOTEN FROM KHACHHANG WHERE MAKH=@MAKH)
	BEGIN
		UPDATE KHACHHANG
			SET HOTEN=@HOTEN
			WHERE MAKH=@MAKH
...
ELSE
	BEGIN
		PRINT 'Không thể update'
	END
END
GO
	--Xóa
ALTER PROCEDURE D_KHACHHANG(@MAKH VARCHAR(5))
AS
	BEGIN
	...
	END
GO
select MAMON,TENMON,DONGIA,TENNM from MONAN,NHOMMON WHERE MONAN.MANM=NHOMMON.MANM
GO
--Thêm dữ liệu bảng MONAN 
CREATE PROCEDURE ADD_MONAN(@MAMON VARCHAR(5), @TENMON NVARCHAR(50),@DONGIA INT,@MANM VARCHAR(5))
AS
	BEGIN
	...
	END
GO
--Sửa dữ liệu bảng Món ăn
CREATE PROCEDURE ALTER_MONAN (@MAMON VARCHAR(5),@TENMON NVARCHAR(50),@DONGIA INT,@MANM VARCHAR(5))
AS
BEGIN
		UPDATE MONAN
		SET TENMON=@TENMON,DONGIA=@DONGIA,MANM=@MANM
		WHERE MAMON=@MAMON
END

GO
--XÓA DỮ LIỆU BẢNG MONAN
CREATE PROCEDURE D_MONAN(@MAMON VARCHAR(5))
AS
	BEGIN
		DELETE FROM MONAN
		WHERE MAMON=@MAMON
...
GO
--SỬA
CREATE PROCEDURE ALTER_NHOMMON (@MANM VARCHAR(5),@TENM NVARCHAR(50))
AS
BEGIN
IF EXISTS(SELECT @MANM,@TENM FROM NHOMMON WHERE MANM=@MANM)
	BEGIN
		UPDATE NHOMMON
			SET TENNM=@TENM
			WHERE MANM=@MANM
...
GO
--XÓA
CREATE PROCEDURE D_NHOMMON(@MANM VARCHAR(5))
AS
	BEGIN 
		IF EXISTS(SELECT * FROM NHOMMON WHERE MANM=@MANM )
			BEGIN
				DELETE FROM NHOMMON
				WHERE MANM=@MANM
			END
...
select KHACHHANG.HOTEN,MONAN.TENMON,CHITIETPHIEUYEUCAU.SOLUONG,MONAN.DONGIA from KHACHHANG,CHITIETPHIEUYEUCAU,MONAN,PHIEUYEUCAU where KHACHHANG.MAKH=PHIEUYEUCAU.MAKH and PHIEUYEUCAU.MAPHIEU = CHITIETPHIEUYEUCAU.MAPHIEU and CHITIETPHIEUYEUCAU.MAMON = MONAN.MAMON
GO
select SUM(SOLUONG*DONGIA) as ThanhTien from CHITIETPHIEUYEUCAU,MONAN where CHITIETPHIEUYEUCAU.MAMON = MONAN.MAMON