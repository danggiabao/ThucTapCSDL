Create database QLNH2
GO
use QLNH2
GO

Create table TAIKHOAN 
(
MA INT iDENTITY PRIMARY KEY,
TEN NVARCHAR(100),
TENDANGNHAP NVARCHAR(50) NOT NULL,
MATKHAU NVARCHAR(30) NOT NULL,
PHANQUYEN INT NOT NULL,
)
GO
CREATE TABLE BAN(
MaBAN VARCHAR(5) PRIMARY KEY NOT NULL,
TENBAN NVARCHAR(10),
SOGHE INT
) 
CREATE TABLE KHACHHANG(
MAKH VARCHAR(5) PRIMARY KEY NOT NULL,
HOTEN NVARCHAR(50) NOT NULL,
)
GO
CREATE TABLE NHANVIEN (
MANV VARCHAR(5) PRIMARY KEY NOT NULL,
HOTEN NVARCHAR(50) NOT NULL,
GIOITINH NVARCHAR(3) CHECK (GIOITINH IN (N'Nam',N'Nữ')),
NGAYSINH DATE,
QUEQUAN NVARCHAR(50)
)
GO
CREATE TABLE HOADON 
(
MAHD VARCHAR(5) PRIMARY KEY NOT NULL,
NGAYLAP DATE,
GIOTHANHTOAN DATETIME,
MANV VARCHAR(5) REFERENCES NHANVIEN(MANV) ON DELETE CASCADE,
MAKH varchar(5) REFERENCES KHACHHANG(MAKH) ON DELETE CASCADE,
)	
GO
CREATE TABLE NHOMMON(
MANM VARCHAR(5) PRIMARY KEY NOT NULL,
TENNM NVARCHAR(50)
)
GO
CREATE TABLE MONAN(
MAMON VARCHAR(5) PRIMARY KEY NOT NULL,
TENMON NVARCHAR(50),
DONGIA INT,
MANM VARCHAR(5) REFERENCES NHOMMON(MANM) ON DELETE CASCADE
)
GO
CREATE TABLE PHIEUYEUCAU(
MAPHIEU VARCHAR(5) PRIMARY KEY NOT NULL,
THOIGIAN TIME,
MAKH VARCHAR(5) REFERENCES KHACHHANG(MAKH) ON DELETE CASCADE,
MABAN VARCHAR(5) REFERENCES BAN(MABAN) ON DELETE CASCADE
)
GO
CREATE TABLE CHITIETPHIEUYEUCAU(
MAPHIEU VARCHAR(5) REFERENCES PHIEUYEUCAU(MAPHIEU)ON DELETE CASCADE,
SOLUONG INT,
MAMON VARCHAR(5) REFERENCES MONAN(MAMON) ON DELETE CASCADE
)
Go
--INSERT
INSERT INTO TAIKHOAN VALUES ('Nam','admin','123456',1);
INSERT INTO TAIKHOAN VALUES ('a','a','a',1);
GO
INSERT INTO NHOMMON VALUES ('001',N'Món đợi');
INSERT INTO NHOMMON VALUES ('002',N'Khai vị');
INSERT INTO NHOMMON VALUES ('003',N'Món chính');
INSERT INTO NHOMMON VALUES ('004',N'Tráng miệng');
INSERT INTO NHOMMON VALUES ('005',N'Ăn sáng');
GO
INSERT INTO MONAN VALUES ('001',N'Khoai tây chiên',30000,'001');
INSERT INTO MONAN VALUES ('002',N'Lạc rang',20000,'001');
INSERT INTO MONAN VALUES ('003',N'Dưa chuột',20000,'001');
INSERT INTO MONAN VALUES ('004',N'Khoai lang chiên',35000,'001');
INSERT INTO MONAN VALUES ('101',N'Súp gà',50000,'002');
INSERT INTO MONAN VALUES ('102',N'Tôm chiên xù',100000,'002');
INSERT INTO MONAN VALUES ('103',N'Mực chiên xù',120000,'002');
INSERT INTO MONAN VALUES ('104',N'Chả giò hải sản',80000,'002');
INSERT INTO MONAN VALUES ('201',N'Cá chép hấp xì dầu',200000,'003');
INSERT INTO MONAN VALUES ('202',N'Ba Ba xào tỏi',200000,'003');
INSERT INTO MONAN VALUES ('203',N'Gà bó xôi',200000,'003');
INSERT INTO MONAN VALUES ('204',N'Bồ câu quay',180000,'003');
INSERT INTO MONAN VALUES ('205',N'Canh cua rau đay',100000,'003');
INSERT INTO MONAN VALUES ('206',N'Canh sườn khoai tây',100000,'003');
INSERT INTO MONAN VALUES ('207',N'Sườn xào chua ngọt',100000,'003');
INSERT INTO MONAN VALUES ('208',N'Thịt bò xào đỗ',170000,'003');
INSERT INTO MONAN VALUES ('301',N'Hoa quả tươi',80000,'004');
INSERT INTO MONAN VALUES ('302',N'Chè hạt sen',50000,'004');
INSERT INTO MONAN VALUES ('303',N'Bánh quy bơ trà xanh',70000,'004');
INSERT INTO MONAN VALUES ('401',N'Phở bò',30000,'005');
INSERT INTO MONAN VALUES ('402',N'Bún chả',30000,'005');
INSERT INTO MONAN VALUES ('403',N'Bún bò',30000,'005');
GO
INSERT INTO BAN VALUES ('001',N'Bàn nhỏ 1',4);
INSERT INTO BAN VALUES ('002',N'Bàn nhỏ 2',4);
INSERT INTO BAN VALUES ('003',N'Bàn nhỏ 3',4);
INSERT INTO BAN VALUES ('004',N'Bàn nhỏ 4',4);
INSERT INTO BAN VALUES ('005',N'Bàn nhỏ 5',4);
INSERT INTO BAN VALUES ('101',N'Bàn vừa 1',8);
INSERT INTO BAN VALUES ('102',N'Bàn vừa 2',8);
INSERT INTO BAN VALUES ('103',N'Bàn vừa 3',8);
INSERT INTO BAN VALUES ('104',N'Bàn vừa 4',8);
INSERT INTO BAN VALUES ('105',N'Bàn vừa 5',8);
INSERT INTO BAN VALUES ('201',N'Bàn to 1',10);
INSERT INTO BAN VALUES ('202',N'Bàn to 2',10);
INSERT INTO BAN VALUES ('203',N'Bàn to 3',12);
INSERT INTO BAN VALUES ('204',N'Bàn to 4',12);
INSERT INTO BAN VALUES ('205',N'Bàn to 5',14);
INSERT INTO BAN VALUES ('206',N'Bàn to 6',14);
INSERT INTO BAN VALUES ('301',N'Bàn VIP 1',8);
INSERT INTO BAN VALUES ('302',N'Bàn VIP 2',12);
INSERT INTO BAN VALUES ('303',N'Bàn VIP 3',16);
INSERT INTO BAN VALUES ('304',N'Bàn VIP 4',20);
GO
INSERT INTO KHACHHANG VALUES ('KH01',N'Lâm Xung');
INSERT INTO KHACHHANG VALUES ('KH02',N'Tống Giang');
INSERT INTO KHACHHANG VALUES ('KH03',N'Lư Tuấn nghĩa');
INSERT INTO KHACHHANG VALUES ('KH04',N'Võ Tòng');
INSERT INTO KHACHHANG VALUES ('KH05',N'Phan Kim Liên');
INSERT INTO KHACHHANG VALUES ('KH06',N'Lữ Bố');
INSERT INTO KHACHHANG VALUES ('KH07',N'Điêu Thuyền');
INSERT INTO KHACHHANG VALUES ('KH08',N'Tào Tháo');
GO

INSERT INTO NHANVIEN VALUES ('NV01',N'Sơn Tùng','Nam','1995-2-2',N'Thái Bình');
INSERT INTO NHANVIEN VALUES ('NV02',N'Chị Dậu',N'Nữ','1992-11-22',N'Hà Nội');
INSERT INTO NHANVIEN VALUES ('NV03',N'Lão Hạc','Nam','5-2-1997',N'Bắc Ninh');
INSERT INTO NHANVIEN VALUES ('NV04',N'Cậu Vàng','Nam','1-2-1995',N'Thái Nguyên');
INSERT INTO NHANVIEN VALUES ('NV05',N'Ông Giáo','Nam','1-7-2000',N'Thái Lan');
INSERT INTO NHANVIEN VALUES ('NV06',N'Hera',N'Nữ','2-4-1992',N'Hàn Quốc');
INSERT INTO NHANVIEN VALUES ('NV07',N'Thuỳ Linh',N'Nữ','12-22-1993',N'Sơn La');
INSERT INTO NHANVIEN VALUES ('NV08',N'Vàng Anh',N'Nữ','11-11-2002',N'Thái Bình');
GO
INSERT INTO HOADON VALUES ('HD01','3-02-2018','6:12','NV01','KH01');
INSERT INTO HOADON VALUES ('HD02','11-02-2018','20:00','NV02','KH02');
INSERT INTO HOADON VALUES ('HD03','01-01-2018','9:30','NV03','KH03');
INSERT INTO HOADON VALUES ('HD04','07-09-2017','12:00','NV04','KH04');
INSERT INTO HOADON VALUES ('HD05','07-09-2017','13:10','NV05','KH05');
INSERT INTO HOADON VALUES ('HD06','07-09-2017','14:03','NV06','KH06');
GO

INSERT INTO PHIEUYEUCAU VALUES ('PH01','11:11','KH01','001');
INSERT INTO PHIEUYEUCAU VALUES ('PH02','22:22','KH02','103');
INSERT INTO PHIEUYEUCAU VALUES ('PH03','12:21','KH03','102');
INSERT INTO PHIEUYEUCAU VALUES ('PH04','21:12','KH04','302');
GO
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH01',2,'001');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH01',4,'204');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH01',2,'301');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH02',1,'002');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH02',5,'202');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH03',2,'101');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH03',1,'208');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH04',1,'004');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH04',3,'104');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH04',3,'205');
INSERT INTO CHITIETPHIEUYEUCAU VALUES ('PH04',1,'303');

Go
--PROC TAIKHOAN
CREATE PROC ThemTK(@TENDANGNHAP NVARCHAR(50),@MATKHAU nvarchar(50),@PHANQUYEN INT) AS 
 BEGIN
	INSERT INTO TAIKHOAN (TENDANGNHAP,MATKHAU,PHANQUYEN )
	VALUES  (@TENDANGNHAP,@MATKHAU,@PHANQUYEN)
 END
GO
CREATE PROC KiemTraDN(@Username VARCHAR(50),@Pass varchar(50)) AS 
BEGIN
	SELECT * FROM TAIKHOAN WHERE TENDANGNHAP = @Username AND MATKHAU = @Pass
END
GO

CREATE PROC DoiMK(@matkhau varchar(50)) AS 
BEGIN
	UPDATE TAIKHOAN
	SET MATKHAU = @matkhau
END
GO

--PROC CÁC BẢNG TRONG CSDL

-- bảng NHANVIEN 
	--Them
CREATE PROCEDURE ADD_NHANVIEN(@MANV VARCHAR(5),@HOTEN NVARCHAR(50),@GIOITINH NVARCHAR(5),@NGAYSINH DATE,@QUEQUAN NVARCHAR(50))
AS
BEGIN
		INSERT INTO NHANVIEN
		VALUES(@MANV,@HOTEN,@GIOITINH,@NGAYSINH,@QUEQUAN)
END
GO
	--Sua
CREATE PROCEDURE ALTER_NHANVIEN(@MANV VARCHAR(5),@HOTEN NVARCHAR(50),@GIOITINH NVARCHAR(5),@NGAYSINH DATE,@QUEQUAN NVARCHAR(50))
AS
BEGIN
	UPDATE NHANVIEN
	SET HOTEN=@HOTEN,GIOITINH=@GIOITINH,NGAYSINH=@NGAYSINH,QUEQUAN=@QUEQUAN
	WHERE MANV=@MANV
			
END	
GO	
	--Xoa
CREATE PROCEDURE D_NHANVIEN(@MANV VARCHAR(5))
AS
BEGIN 
	DELETE FROM NHANVIEN
	WHERE MANV=@MANV
END		
GO
--bảng HOADON
	--Thêm
create PROCEDURE ADD_HOADON (@MAHD VARCHAR(5) ,@NGAYLAP DATE,@GIOTHANHTOAN DATETIME,@MANV VARCHAR(5),@MAKH VARCHAR(5))
AS
BEGIN
	INSERT INTO HOADON
	VALUES(@MAHD,@NGAYLAP,@GIOTHANHTOAN,@MANV, @MAKH)
END
GO
	--Sủa
CREATE PROCEDURE ALTER_HOADON(@MAHD VARCHAR(5) ,@NGAYLAP DATE,@GIOTHANHTOAN DATETIME,@MANV VARCHAR(5),@MAKH varchar(5) )
AS
BEGIN
	UPDATE HOADON
	SET NGAYLAP=@NGAYLAP,GIOTHANHTOAN=@GIOTHANHTOAN,MANV=@MANV,MAKH=@MAKH
	WHERE MAHD=@MAHD
END
GO
	--Xóa
CREATE PROCEDURE D_HOADON(@MAHD VARCHAR(5))
AS
BEGIN 
				DELETE FROM HOADON
				WHERE MAHD=@MAHD
END
GO

-- bảng KHACHHANG
--thêm
CREATE PROCEDURE ADD_KHACHHANG (@MAKH VARCHAR(5),@HOTEN NVARCHAR(50))
AS
BEGIN
IF NOT EXISTS(SELECT @MAKH,@HOTEN FROM KHACHHANG WHERE MAKH=@MAKH)
	BEGIN
		INSERT INTO KHACHHANG(MAKH,HOTEN)
				VALUES(@MAKH,@HOTEN)
	END
ELSE
	BEGIN
		PRINT 'Đã tồn tại'
    END
END
GO

	--Sua
CREATE PROCEDURE ALTER_KHACHHANG (@MAKH VARCHAR(5),@HOTEN NVARCHAR(50))
AS
BEGIN
IF EXISTS(SELECT @MAKH,@HOTEN FROM KHACHHANG WHERE MAKH=@MAKH)
	BEGIN
		UPDATE KHACHHANG
			SET HOTEN=@HOTEN
			WHERE MAKH=@MAKH
	END
ELSE
	BEGIN
		PRINT 'Không thể update'
	END
END
GO
	--Xóa

CREATE PROCEDURE D_KHACHHANG(@MAKH VARCHAR(5))
AS
	BEGIN 
		IF EXISTS(SELECT MAKH FROM KHACHHANG WHERE MAKH=@MAKH )
			BEGIN
				DELETE FROM KHACHHANG
				WHERE MAKH=@MAKH
			END
		ELSE
			BEGIN
				PRINT 'Không tồn tại!'
			END
	END
GO

--bảng MONAN 
	--thêm
CREATE PROCEDURE ADD_MONAN
@MAMON VARCHAR(5), 
@TENMON NVARCHAR(50),
@DONGIA INT,
@MANM VARCHAR(5)

AS
BEGIN
IF NOT EXISTS(SELECT @MAMON,@TENMON,@DONGIA,@MANM FROM MONAN WHERE MAMON=@MAMON OR TENMON=@TENMON OR DONGIA=@DONGIA OR MANM=@MANM)
			BEGIN
				INSERT INTO MONAN(MAMON,TENMON,DONGIA,MANM)
				VALUES(@MAMON,@TENMON,@DONGIA,@MANM)
			END
		ELSE
			BEGIN
				PRINT 'ALREADY EXIST'
				ROLLBACK TRANSACTION
            END
END
	--Sửa 
CREATE PROCEDURE ALTER_MONAN (@MAMON VARCHAR(5),@TENMON NVARCHAR(50))
AS
BEGIN
IF EXISTS(SELECT @MAMON,@TENMON FROM MONAN WHERE MAMON=@MAMON)
	BEGIN
		UPDATE MONAN
			SET TENMON=@TENMON
			WHERE MAMON=@MAMON
	END
ELSE
	BEGIN
		PRINT 'Không thể update'
	END
END
GO
	--XÓA DỮ LIỆU BẢNG MONAN
CREATE PROCEDURE D_MONAN(@MAMON VARCHAR(5))
AS
	BEGIN 
		IF EXISTS(SELECT * FROM MONAN WHERE MAMON=@MAMON )
			BEGIN
				DELETE FROM MONAN
				WHERE MAMON=@MAMON
			END
		ELSE
			BEGIN
				PRINT 'Không tồn tại'
			END
	END

	--Thêm dữ liệu bảng NHOMMON

CREATE PROCEDURE ADD_NHOMMON

@MANM VARCHAR(5),
@TENNM NVARCHAR(50)
AS
BEGIN
IF NOT EXISTS(SELECT @MANM,@TENNM FROM NHOMMON WHERE MANM=@MANM OR TENNM=@TENNM)
			BEGIN
				INSERT INTO NHOMMON(MANM,TENNM)
				VALUES(@MANM,@TENNM)
			END
		ELSE
			BEGIN
				PRINT 'ALREADY EXIST'
				ROLLBACK TRANSACTION
            END
END
	--SỬA
CREATE PROCEDURE ALTER_NHOMMON (@MANM VARCHAR(5),@TENM NVARCHAR(50))
AS
BEGIN
IF EXISTS(SELECT @MANM,@TENM FROM NHOMMON WHERE MANM=@MANM)
	BEGIN
		UPDATE NHOMMON
			SET TENNM=@TENM
			WHERE MANM=@MANM
	END
ELSE
	BEGIN
		PRINT 'Không thể update'
	END
END
GO
	--XÓA
CREATE PROCEDURE D_NHOMMON(@MANM VARCHAR(5))
AS
	BEGIN 
		IF EXISTS(SELECT * FROM NHOMMON WHERE MANM=@MANM )
			BEGIN
				DELETE FROM NHOMMON
				WHERE MANM=@MANM
			END
		ELSE
			BEGIN
				PRINT 'Không tồn tại'
			END
	END



--Thêm dữ liệu BAN 

CREATE PROCEDURE ADD_BAN

@MABAN VARCHAR(5) ,
@TENBAN NVARCHAR(10),
@SOGHE INT
AS
BEGIN
IF NOT EXISTS(SELECT @MABAN,@TENBAN,@SOGHE FROM BAN WHERE MABAN=@MABAN OR TENBAN=@TENBAN OR SOGHE=@SOGHE)
			BEGIN
				INSERT INTO BAN(MABAN,TENBAN,SOGHE)
				VALUES(@MABAN,@TENBAN,@SOGHE)
			END
		ELSE
			BEGIN
				PRINT 'ALREADY EXIST'
				ROLLBACK TRANSACTION
            END
END
	--SỬA
CREATE PROCEDURE ALTER_BAN(@MABAN VARCHAR(5), @TENBAN NVARCHAR(10),@SOGHE INT)
AS
BEGIN
	UPDATE BAN
	SET MABAN = @MABAN,TENBAN =@TENBAN, SOGHE = @SOGHE
	WHERE MABAN=@MABAN
			
END	
GO	
	--XÓA
CREATE PROCEDURE D_BAN(@MABAN VARCHAR(5))
AS
BEGIN 
	DELETE FROM BAN
	WHERE MaBAN=@MABAN
END		
GO

--bảng PHIEUYEUCAU
	--THÊM
CREATE PROCEDURE ADD_PHIEUYEUCAU
@MAPHIEU INT,
@THOIGIAN TIME,
@MAKH VARCHAR(5),
@MABAN VARCHAR(5) 
AS
BEGIN
IF NOT EXISTS(SELECT @MAPHIEU,@THOIGIAN,@MAKH FROM PHIEUYEUCAU WHERE MAPHIEU=@MAPHIEU OR THOIGIAN=@THOIGIAN OR MAKH=@MAKH)
			BEGIN
				INSERT INTO PHIEUYEUCAU(MAPHIEU,THOIGIAN,MAKH)
				VALUES(@MAPHIEU,@THOIGIAN,@MAKH)
			END
		ELSE
			BEGIN
				PRINT 'ALREADY EXIST'
				ROLLBACK TRANSACTION
            END
END
	--SỬA
CREATE PROCEDURE ALTER_PHIEUYEUCAU(@MAPHIEU INT,@THOIGIAN TIME,@MAKH VARCHAR(5),@MABAN VARCHAR(5))
AS
BEGIN
	UPDATE PHIEUYEUCAU
	SET MAPHIEU = @MAPHIEU,THOIGIAN =@THOIGIAN,MAKH = @MAKH,MABAN =@MABAN
	WHERE MAPHIEU=@MAPHIEU
			
END	
GO	
	--XÓA
CREATE PROCEDURE D_PHIEUYEUCAU(@MAPHIEU INT)
AS
BEGIN 
	DELETE FROM PHIEUYEUCAU
	WHERE MAPHIEU=@MAPHIEU
END		
GO


--bảng CHITIETPHIEUYEUCAU
	--THÊM
CREATE PROCEDURE ADD_CHITIETPHIEUYEUCAU
@MAPHIEU varCHAR(5),
@SOLUONG INT,
@MAMON varCHAR(5)
AS
BEGIN
IF NOT EXISTS(SELECT @MAPHIEU,@SOLUONG,@MAMON FROM CHITIETPHIEUYEUCAU WHERE MAPHIEU=@MAPHIEU OR SOLUONG=@SOLUONG OR MAMON=@MAMON)
			BEGIN
				INSERT INTO CHITIETPHIEUYEUCAU
				VALUES(@MAPHIEU,@SOLUONG,@MAMON)
			END
		ELSE
			BEGIN
				PRINT 'ALREADY EXIST'
				ROLLBACK TRANSACTION
            END
END
	--SỬA
CREATE PROCEDURE ALTER_CHITIETPHIEUYEUCAU(@MAPHIEU VARCHAR(5),@SOLUONG INT,@MAMON VARCHAR(5))
AS
BEGIN
	UPDATE CHITIETPHIEUYEUCAU
	SET MAPHIEU = @MAPHIEU,SOLUONG = @SOLUONG,MAMON =@MAMON
	WHERE MAPHIEU=@MAPHIEU
			
END	
GO	
	--XÓA
CREATE PROCEDURE D_CHITIETPHIEUYEUCAU(@MAPHIEU VARCHAR(5))
AS
BEGIN 
	DELETE FROM CHITIETPHIEUYEUCAU
	WHERE MAPHIEU=@MAPHIEU
END		
GO




